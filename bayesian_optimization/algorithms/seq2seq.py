...
from algorithms.SEQ2SEQ.seq2seqdisaggregator import Seq2SeqDisaggregator
...
def seq2seq(dataset_path, train_building, train_start, train_end, val_building, val_start, val_end,
    ...
    model = Seq2SeqDisaggregator(patience=patience,
                                optimizer=optimizer,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Seq2Seq Disaggregator')
    parser.add_argument('--datapath', '-d', type=str, required=True, help='hd5 filepath')
    ...
    with open('seq2seq_json.json', 'a+') as outfile:
